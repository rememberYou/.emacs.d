...
In order to avoid any problems with accents.
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defhydra hydra-go-to-file (:color blue)
    ...
    ("cn" (find-file (format "%s/neofetch/config" xdg-config)))
    ...
    ("ct" (find-file (format "%s/termite/config" xdg-config)))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package window
    ...
    (defun my/vsplit-last-buffer ()
      "Splits the window vertically and display the previous buffer."
      (interactive)
      (split-window-vertically)
      (other-window 1 nil)
      (switch-to-next-buffer))
    (defun my/hsplit-last-buffer ()
      "Splits the window horizontally and display the previous buffer."
      (interactive)
