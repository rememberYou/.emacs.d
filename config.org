...
default behavior when multiple windows are present.
...
of the window I want to move to. Most people use =ace-window=, but I prefer
=switch-window= which displays the window number while hiding its content. I find
...
content. With =winner-undo= and =winner-redo= I can easily do that.
