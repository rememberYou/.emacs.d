...
using =lsp-mode= as a client for LSP servers and =ccls= as server.

*NOTE:* some of you might choose [[https://github.com/joaotavora/eglot][eglot]] as LSP.

Of course, to use =ccls= with GNU Emacs, you must first install it with the
...
I often use LaTeX in SRC blocs, that explain the configuration below:
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package ob-latex
    ...
    (let ((dvipng (alist-get 'dvipng org-preview-latex-process-alist)))
      (push (cons 'dvipng (plist-put dvipng
...
I recommend to use =lsp-mode= as a client for LSP servers and =pyls= as
server. To use =pyls= with GNU Emacs, you must first install it with the package
