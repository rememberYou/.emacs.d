...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package dired-x
    ...
    (defun my/dired-revert-after-cmd (command &optional output error)
      (revert-buffer))
    :config
    (advice-add 'dired-smart-shell-command :after #'my/dired-revert-after-cmd))
#+END_SRC
...
*** Execute a command after saved file

Automatic executes a specific bash command after saved a specific file.
...
Jumps to mark, and pop a new position for mark off the ring. Notice that it
doesn't affect global mark ring.
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package org-agenda
    ...
    (org-agenda-files '("~/Sync/shared/.personal"))
    ...
    (org-default-notes-file "~/Sync/shared/.personal/organizer.org")
    (org-directory "~/Sync/shared/personal")
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package ob-plantuml :ensure nil :after org)
