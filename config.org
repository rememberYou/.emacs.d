...
In order to follow the [[https://specifications.freedesktop.org/basedir-spec/basedir-spec-latest.html][XDG base directory specification]], I place the =cache= and
the =bookmark= files to =/home/$USER/cache=. However, these files will only be
created if the =projectile= folder is created in the =cache= folder.

#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package projectile
    :defer 1
    :custom
    (projectile-cache-file (expand-file-name (format "%s/projectile/cache" xdg-cache)))
    (projectile-completion-system 'ivy)
    (projectile-enable-caching t)
    (projectile-known-projects-file (expand-file-name (format "%s/projectile/bookmarks" xdg-cache)))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package ob-plantuml
    :ensure nil
    :after org
    :custom (org-plantuml-jar-path (expand-file-name "~/.local/lib/plantuml.jar")))
