...
#+begin_src emacs-lisp
  (use-package flyspell
    ...
    (flyspell-abbrev-p t)
    (flyspell-default-dictionary "en-US")
...
#+begin_src emacs-lisp
  (use-package ispell
    ...
    (defun my/switch-language ()
      "Switches between the English and French for ispell, flyspell, and LanguageTool."
      (interactive)
      (let* ((current-dictionary ispell-current-dictionary)
        ...
        (if (string= new-dictionary "fr_BE")
            (progn
              ...
              (lui-flyspell-change-dictionary "fr_BE"))
          (progn
            (setq lsp-ltex-language "en-US")
            (lui-flyspell-change-dictionary "en_US")))
        (flyspell-buffer)
...
The package listing menu has a simplistic interface. The [[https://github.com/Malabarba/paradox][paradox]] package
improves the display of this interface, provides different mode line information
and other features.
...
=auth-sources= variable.

To add your IRC account in the =authinfo.gpg= file, add in the following line with
your credentials:
...
#+begin_src emacs-lisp
  (use-package circe
    ...
    (defun my/circe-fetch-password (&rest params)
      "Fetchs the NickServ password for an username."
      (require 'auth-source)
    ...
    (defun my/circe-nickserv-password (server)
      "Fetchs the NickServ password for the Libera Chat."
